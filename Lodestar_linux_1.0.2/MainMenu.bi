...
declare sub StLogo
declare sub ReadOpt
declare sub SoundLoad(mus as Mix_Chunk ptr,m_name as string)
declare sub PlaySound(mus as Mix_Chunk ptr)
Declare sub PlayMusic(music as Mix_Music ptr,names as string,chanel as byte)
declare sub Music_End(music as Mix_Music ptr)
...
sub MainMenu
  ...
  if setting_mus=1 then
     PlayMusic(m_fon,"sounds/mus_menu.ogg",-1)
  endif
        ...
        elseif y=445 then   
            if m1="OFF" then 
               setting_mus=0'*** 
                ...
                setting_mus=1
                if m_fon=0 then PlayMusic(m_fon,"sounds/mus_menu.ogg",-1)
            endif
            if k1="Keyboard" then
                setting_key=0
            else 
...
bload "img/logo3.bmp",@img(0)
    setting_mus=1
    dim fon as Mix_Music ptr=0
    fon = Mix_LoadMUS ("sounds/mus_logo.ogg")
    Mix_PlayMusic(fon, 0) 
...
'********************
sub SoundLoad(mus as Mix_Chunk ptr,m_name as string)
if setting_mus=1 then
    if mus=NULL then mus = Mix_LoadWAV(m_name)
endif
end sub
'********************

'********************
sub PlaySound(mus as Mix_Chunk ptr)
if setting_mus=1 then  
    if mus<>NULL then Mix_PlayChannel (-1, mus, 0)
endif
end sub
'********************
sub PlayMusic(music as Mix_Music ptr,names as string,chanel as byte)
   if music=NULL then
   music = Mix_LoadMUS (names)
    Mix_PlayMusic (music, chanel) 
    endif
end sub    
'********************
sub Music_End(music as Mix_Music ptr)
if setting_mus=1 then
   Mix_HaltMusic
   Mix_FreeMusic(m_fon)
   Mix_FreeChunk(m_st)
   Mix_FreeChunk(m_st2)
   m_fon=NULL:m_st=NULL:m_st2=NULL  
endif
end sub 
'********************
sub Fmod_End()
